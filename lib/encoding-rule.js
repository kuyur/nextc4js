...
var EncodingRuleMultibyte = function(segments, mappingBuffer, byte, unknownChar) {
  ...
  if (mappingBuffer) {
    ...
    } else if (goog.isString(mappingBuffer)) {
      if (mappingBuffer.startsWith(consts.EMBEDDED_BASE64_PREFIX)) {
        var encoded = mappingBuffer.substring(consts.EMBEDDED_BASE64_PREFIX.length);
