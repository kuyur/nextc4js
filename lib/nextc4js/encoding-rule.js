 ...
 * @extends {EncodingRule}
 */
var EncodingRuleMultiplebyte = function(segments, mappingBuffer, whiteSpace) {
  EncodingRule.call(this);

  this.segments_ = new segment.Segments(segments);
  this.mappingBuffer_ = mappingBuffer;
  this.whiteSpace_ = whiteSpace || 0x20;
  this.whiteSpaceLength_ = this.whiteSpace_ <= 0xFF ? 1 : 2;
};
 ...
 * @type {number}
 */
EncodingRuleMultiplebyte.prototype.whiteSpace_;
...
EncodingRuleMultiplebyte.prototype.test = function(buffer) {
  ...
  for (var i = 0; i < len; ++i) {
    ...
    case segment.Reference.BUFFER:
      result += seg.getBytes() === 1 ? 1 :
        this.getCharBytes_(this.mappingBuffer_[chr - seg.getBegin() + seg.getOffset()]);
      break;
