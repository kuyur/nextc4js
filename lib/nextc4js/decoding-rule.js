 ...
 *   the decoding rule.
 */
DecodingRule.prototype.parse = function(buffer) {
 ...
 */
DecodingRule.prototype.parse2 = function(buffer) {
  var result = [];
  var offset;
  for (var i = 0, j = 0, len = buffer.length; i < len; ++j) {
    offset = this.consume2(buffer, i, result, j);
    if (offset === 0) {
      return null;
    }
    i += offset;
 ...
 * @return {number} bytes consumed.
 */
DecodingRule.prototype.consume2 = goog.abstractMethod;
 ...
 * @return {number} The count of code points.
 */
DecodingRuleUTF16LE.prototype.test = function(buffer) {
  if (!buffer) {
    return -1;
  }
  if (buffer.length === 0) {
 ...
 * @return {number}
 */
DecodingRuleUTF16LE.prototype.consume2 = function(buffer, offset, result, pos) {
 ...
 * @return {number} The count of code points.
 */
DecodingRuleUTF16BE.prototype.test = function(buffer) {
  if (!buffer) {
    return -1;
  }
  if (buffer.length === 0) {
...
DecodingRuleUTF16BE.prototype.consume = function(buffer, offset, result, pos) {
  var chr = (buffer[offset] << 8) | buffer[offset + 1] << 8, consumed = 2;
  if (chr >= 0xD800 && chr <= 0xDBFF) { // high byte matched
 ...
 * @return {number}
 */
DecodingRuleUTF16BE.prototype.consume2 = function(buffer, offset, result, pos) {
  var length = buffer.length;
  if (offset + 1 >= length) { // invalid
    return 0;
  }
...
/**
 * UTF-8 decoding rule. The maximum code point for UTF-8 is 0x7FFFFFFF. It is
 ...
 * @return {number} The count of code points.
 */
DecodingRuleUTF8.prototype.test = function(buffer) {
  if (!buffer) {
    return -1;
  }
  if (buffer.length === 0) {
 ...
 * @return {number}
 */
DecodingRuleUTF8.prototype.consume2 = function(buffer, offset, result, pos) {
  var length = buffer.length;
  if ((0x80 & buffer[offset]) === 0) { // ASCII
    result[pos] = buffer[offset];
    return 1;
...
var DecodingRuleMultibyte  = function(options) {
  ...
  if (options && goog.isArray(options)) {
    options.forEach(function(option) {
      var condition = Condition.build(option['condition']);
      if (condition) {
 ...
 * @return {number} The count of code points.
 */
DecodingRuleMultibyte.prototype.test = function(buffer) {
...
/**
 * It is safe to accelerate the converting without checking buffer boundary.
 * @private
 * @param {Uint8Array} buffer
 * @param {number} offset
 * @param {number} bytes
 ...
 * @return {number}
 */
DecodingRuleMultibyte.prototype.consume2 = DecodingRuleMultibyte.prototype.consume;

exports.DecodingRuleMultibyte = DecodingRuleMultibyte;
exports.DECODING_RULE_UTF16LE = new DecodingRuleUTF16LE();
exports.DECODING_RULE_UTF16BE = new DecodingRuleUTF16BE();
exports.DECODING_RULE_UTF8 = new DecodingRuleUTF8();
