...
const utils = require('./buffer-utils');
...
/**
 * Decode the buffer and return Unicode code points. The BOM will be kept in
 * the output buffer. This function is a shortcut for the code in example.
 * @example
 ...
 * additional BOM will be attached to the encoded buffer even UTF-8/UTF-16LE/UTF-16BE
 ...
 */
Context.prototype.decodeAsString = function(buffer, decoderName, opt_offset) {
  var dec = this.getDecoder(decoderName);
  if (!dec) {
    throw `Error: decoder ${decoderName} not found in the context.`;
  }
  ...
  if (decoded && decoded.codePointAt(0) === consts.UNICODE_BYTE_ORDER_MARK) {
 ...
 * @return {Uint8Array} encoded buffer (binary data) which can be written to file directly.
 */
Context.prototype.encodeFromString = function(str, encoderName, opt_appendBOM) {
  var enc = this.getEncoder(encoderName);
  if (!enc) {
    throw `Error: encoder ${encoderName} not found in the context.`;
  }
  ...
  if (opt_appendBOM && (enc === encoder.UTF8 || enc === encoder.UTF16LE || enc === encoder.UTF16BE)) {
    // append the BOM
